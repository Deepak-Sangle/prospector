...
- `tools/` — AI SDK `tool({description, inputSchema, execute})` definitions with Zod v4 schemas, returning plain strings. All currently **stubs** over in-memory data:
  - `monitors.ts`: `create_monitor`, `list_monitors`, `delete_monitor`
  - `leads.ts`: `get_recent_leads`, `draft_reply`
- `deps` (`ProspectorDeps`: Slack `client`, user/channel/thread IDs) is threaded through for future tools that need Slack API access (reactions, posting) — pattern inherited from the template.
...
- The agent tools in `agent/tools/` are still in-memory stubs — wiring them to Prisma is the next step.
